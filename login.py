...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user[3], password):  # Assuming the hashed password is in the 4th column
            ...
            # Redirect to the main page served by the server.js
            return redirect("http://localhost:3000/")  # Adjust the URL and port as per your server.js setup
        else:
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(url_for('login'))  # Redirect to the landing page (login page)
...
# Index route (homepage after login) - If needed for Flask routing, but now you will redirect to server.js
@app.route('/index')
def index():
